...
function Option(_a) {
    ...
    return (<li className={"border border-top-0 border-muted px-2 cursor-pointer bg-white text-dark hover-primary"} style={{ listStyleType: "none", lineHeight: "3rem" }} onClick={onClickHandler}>
            {text}
        </li>);
}
...
function SelectFilter(_a) {
    ...
    return (<div className="position-relative">
            <react_bootstrap_1.Form.Control type="search" value={filter} onChange={onChangeFilter} placeholder="Type to filter" onFocus={showDropDown} onBlur={hideDropDown} role="filter"/>
            <react_bootstrap_1.Collapse in={isDropDownVisible}>
                <ol className={"p-0 z-100 position-absolute w-100"}>
                    {filteredOptions.map(function (_a) {
            var value = _a.value, text = _a.text;
            return (<Option key={value} value={value} text={text} onClick={onOptionClick}/>);
        })}
                </ol>
