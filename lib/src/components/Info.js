...
function Overlay(title, body) {
    return (<react_bootstrap_1.Popover id="info-overlay">
            <react_bootstrap_1.Popover.Header as="h3">{title}</react_bootstrap_1.Popover.Header>
            <react_bootstrap_1.Popover.Body>{body}</react_bootstrap_1.Popover.Body>
...
function Info(_a) {
    ...
    return (<react_bootstrap_1.OverlayTrigger trigger="click" placement="right" overlay={Overlay(title, children)}>
            <react_bootstrap_1.Button variant="link" className={"text-sm text-white font-serif h-100 " + className} tabIndex="-1" role="button">
                help
